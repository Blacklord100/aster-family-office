...
async function main() {
 ...
 const folder = await mkdtemp(join(tmpdir(),'aster-recovery-')); let restored, created = false;
 try {
  ...
  const contexts = {
   ...
   app_jobs: [['result',row=>'result:'+row.organization_id+':'+row.id]],
  ...
  for(const [table,fields] of Object.entries(contexts))for(const row of JSON.parse(tables[table]))for(const [field,context] of fields) {
   if(!row[field])continue;const value=Buffer.from(row[field].slice(2),'hex'), decipher=createDecipheriv('aes-256-gcm',appKey,value.subarray(1,13));
   decipher.setAAD(Buffer.from(context(row)));decipher.setAuthTag(value.subarray(13,29));Buffer.concat([decipher.update(value.subarray(29)),decipher.final()]);decrypted++;
  }
  console.log(JSON.stringify({result:'passed',tables:order.length,records:Object.values(tables).reduce((sum,text)=>sum+JSON.parse(text).length,0),decryptedRecords:decrypted,encryptedBytes:envelope.length,elapsedMs:Date.now()-started,format:'bounded development SQL snapshot; production pg_dump/age drill still separate'}));
 } finally {
  ...
  await source.end();await rm(folder,{recursive:true,force:true});
 }
}
main().catch(()=>{console.error('Native recovery drill failed; credentials and record contents suppressed.');process.exitCode=1;});
