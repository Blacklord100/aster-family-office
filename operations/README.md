...
docker compose --profile tls up -d web worker mailbox-worker archive-worker report-obligations-worker caddy
```
...
See [local source archives](document-archive.md) for durable source copies, email snapshots, destination permissions and archive-volume backup requirements.
