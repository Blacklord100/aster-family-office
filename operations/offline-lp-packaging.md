...
The revised implementation adds bounded nested-EML decoding, separate native text/layout/source-image views, shared document tools in both modes, source-ID evidence, broader owner/date/unit validation, operating-news handling and permanent-input retry controls. The [document-tool decision and research record](document-tools-research.md) explains these changes, exact model capabilities and the separate synthetic image probe. Revised four-configuration qualification is pending at this document update; reader tests and a successful image probe are not complete corpus results. Preserve the original sources, gold labels and baseline outputs; measure the new processing revision separately, then qualify it on independently adjudicated client-like documents. Unknown currencies, dates and weights stay unknown, and financial posting still requires review.
