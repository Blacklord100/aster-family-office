...
| --- | --- | --- |
| Runtime | Standalone Next app; separate document, folder, archive, mailbox, delivery and reporting workers; PostgreSQL; Python/OCR processor; Ollama; Caddy template | Qualify the exact release on the chosen target. CI image builds and isolated probes do not certify the complete client deployment. |
...
The **historical 100-email baseline** returned 60 of 90 expected facts per configuration. That processing revision omitted readable NAV/consolidation tables and legitimate operating news, and rejected nested email attachments. Those results remain the baseline; they are not measurements of the revised implementation. Packaging alone does not establish extraction accuracy.

The revised implementation adds bounded nested-EML decoding, separate native text/layout/source-image views, shared document tools in both modes, source-ID evidence, broader owner/date/unit validation, operating-news handling and permanent-input retry controls. The [document-tool decision and research record](document-tools-research.md) explains these changes and exact model capabilities. **Gemma workflow, Gemma agentic, Qwen workflow and Qwen agentic each recovered 90/90 exact fact observations with zero unsupported persisted facts and 94/94 readable-job relevance decisions**. Each cell finalized 97 unique jobs, including **three explicit encrypted-PDF blocks**; blocked inputs are not successful extractions. All 388 processor requests/responses and 1,632 model transport records, including 121 image-bearing chats, passed integrity audits. See the [completed upgrade results and evidence](../benchmark/mailroom-v1/UPGRADE-RESULTS.md).

The main corpus's deterministic source-rule floor is already **90/90 without model inference**. Its perfect combined result does not establish independent LLM accuracy or vision's incremental contribution. Separate ten-source, model-only development probes had zero rule discoveries and reached **20/20 positive observations plus 20/20 negative controls across four configurations**, after tuning on those same fixtures. Earlier failed versions remain preserved. Raw responses in all four main cells still contain wrong ownership/scope, withdrawn values and malformed amounts; schema and grounding checks rejected them. Preserve sources, gold and every attempted result, then qualify the chosen release on independently adjudicated client-like documents. Unknown currencies, dates and weights stay unknown, and financial posting still requires review. Independent client-like and Linux appliance qualification remain release gates; they do not negate completion of this extraction repair.

The [optional offline document-layout stage](offline-document-layout.md) remains a separate candidate. No Docling dependency or additional model download has been introduced. Adopt it only if measured extraction improves and the additional assets fit the offline resource and licensing inventory.

## Offline model and tool qualification for this revision

Pin the exact model digest and runtime settings in each release. The installed `gemma4:e4b-m3` alias has verified local vision metadata and passed a synthetic image request. The installed `qwen3-aster-cpu:1.7b` alias is text-only and uses CPU inference. These findings apply to those aliases and digests; another Gemma/Qwen release or conversion needs its own qualification. Unsupported image capability produces an explicit text/layout fallback with no model substitution. Both modes have the same local privacy boundary: read-only tools operate on the authorized document, and this implementation sends page images only to the verified local adapter. An image-only proposal without independently readable evidence cannot become an accepted fact. Capability discovery is not extraction certification.

The new engine-inspection panel passed actual desktop/mobile browser checks using synthetic password/TOTP login and installed Gemma/Qwen metadata. Its tested isolated revision **05ed07b** was integrated into main through **0af1882** after the benchmark; the extraction evidence remains pinned to ab7c919. The panel shows supported, unsupported, unknown and deployment-disabled image states separately, with the synthetic generation test explicitly labelled text connectivity/schema only. Its local digest is **observed at inspection, not an immutable weight pin for later jobs**. The displayed resource limits are deployment/processor ceilings; there are no per-profile overrides. Release packaging must still verify exact loaded model assets and fail closed on substitution. Full native validation passed 588 Python tests and 326 Node tests (35 integration skips), lint, TypeScript, the production Next build and service bundles; main build/TypeScript/service bundles passed after integration. These checks do not validate Linux containers or network denial.

The local adapter now requests a **16,384-token context** and at most 3,200 output tokens, with an 11,500-byte combined system/user/schema text limit before generation. Source windows are at most 2,800 UTF-8 bytes; auxiliary layout is at most 2,400 bytes per request. Image tokenization adds model-specific resource use and is not covered by that text-byte limit. Default document limits permit six rendered pages and 8 MiB of images, while planner actions, structured calls, page attempts and the independent 590-second document deadline remain bounded. Certify peak RAM/VRAM, maximum source/image cases, OCR, timeouts and incomplete-coverage reporting on the target Linux host. The existing 8-GiB Ollama Compose limit is a placeholder, not proof that a 16K vision workload fits. See the [processor bounds and configuration](../processor/README.md#bounds-and-confidentiality).

Treat the new reader, source-ID prompts, validation rules and context settings as a processing migration even when model weights stay unchanged. Existing explicitly configured environment limits remain in force; review them against the new defaults and expected document volume. Drain old workers before activating a tested release, retain prior job provenance and accepted facts, and offer a separate reviewed reprocessing comparison. Never silently overwrite accepted holdings or claim that the old 60/90 run validates this revision. Ship all model blobs/support assets and the already-pinned PDFium/Pillow dependencies in the signed release images. Linux Tesseract and its approved language data must also be present before installation; the current Dockerfile installs English during the connected build, not during document processing. Test first boot and every failure path with internet access denied.
...
For source retention, include the optional archive worker and its private archive volume shared with web. It preserves originals and renders readable email copies through the local processor without an LLM or external service. Inventory, encrypt at the storage layer, back up and restore this plaintext export volume separately from PostgreSQL; do not attach a cloud-sync service in an internet-free installation. Follow the [archive setup, retention and verification guide](document-archive.md), and test file hashes against recovered database receipts after a restore.
...
| Independent extraction qualification | Adjudicate a separate client-like set with unseen issuers, document templates, languages and scan quality. Measure model additions beyond source rules, rejected raw candidates, abstention and correction burden; do not treat a tuned synthetic 90/90 result as production accuracy. |
