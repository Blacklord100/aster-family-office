...
No container publishes a host port. Two controller-managed systemd sockets accept
IPv4 HTTP/HTTPS on ports 80/443 and pass their listening descriptors to a small
nonroot relay. The relay forwards opaque bytes to Caddy's protected Unix sockets;
TLS still terminates in Caddy. It records the actual TCP peer in a PROXY prefix,
so Caddy can overwrite the client-address header used for authentication and
...
The signed manifest selects `systemd-unix-v1` ingress. Runtime socket files live
under `<installation-root>/run/ingress-sockets`, owned by UID/GID 10001 with mode
0700, and are recreated rather than included in encrypted backups. The controller
starts listeners after the fleet starts, then checks real HTTPS before reopening
writes. Backup, stop and release replacement disable socket activation and stop
the relay before stopping Caddy. Use `asterctl stop` and `asterctl resume` for
these operations; direct Compose commands do not manage the host listeners.
