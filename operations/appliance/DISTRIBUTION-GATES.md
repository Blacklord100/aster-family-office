...
to obtain it. The initial review inspected repository code, upstream recipes and
small OCI metadata responses. Later Linux runs built application images and
collected the bounded processor and sharp source sets described below. No
extracted Ollama CPU inventory or complete target-server qualification has run.
Existing collectors must still produce passing receipts for the exact candidate;
their presence in the repository is not an executed result.
...
| sharp/libvips | [collect-native-sources.py](../../tools/release/collect-native-sources.py) implements exact app-image/native-file verification with a reviewed closure of 29 native archives and 350 Cargo archives, recipes, patches and notices. [Linux run 34713572531](https://github.com/Blacklord100/aster-family-office/actions/runs/34713572531) retained all 379 archives. The subsequent source-only reread validated 738 original-notice records, 2,816 complete older-source file mappings and 16 original MPL source headers. | Four original-notice gaps remain in `block`, `malloc_buf`, `objc-foundation` and `objc_id`; see the [exact versions and hashes](../../licenses/native/sharp-libvips-1.3.3/README.md). The complete graph therefore still fails verification. Resolving eight filename/provenance omissions does not establish the remaining license evidence, an executed independent Linux offline receipt, or a narrower actual build graph. This gate also excludes other OS/npm/Python material and reproducible rebuild qualification. |
...
| Custom processor Tesseract, libtiff and Pillow | [upstream-sources.json](../../processor/runtime/upstream-sources.json) pins Tesseract 5.5.3, libtiff 4.7.2 and Pillow 12.3.0. The separate Docker `source-evidence` target now exports the original archives, pinned build wheels, authentication material, every reviewed backport, build recipe, compiler configuration, package inventory and regression evidence. The [collector](scripts/collect-processor-sources.py) compares reviewed inputs and checks the export digest marker, runtime manifest and installed native/Pillow files inside the exact processor image; staging checks this evidence again against the raw security manifest. Only the small marker enters the service image. | The export and exact-image attestation passed at `25118b8` in [Linux run 34710535770](https://github.com/Blacklord100/aster-family-office/actions/runs/34710535770); its 46 retained material files and 113 installed identities were independently reverified. Require a matching receipt for every later candidate. Map supporting codecs, Leptonica, the retained OCR language data, CPython, distro packages and other wheels separately. This bounded export does not establish reproducible compilation or complete distribution clearance. |
