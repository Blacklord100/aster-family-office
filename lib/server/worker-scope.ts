...
export async function claimDocumentJob(
  ...
  const result = await database.query<{
    ...
    `UPDATE app_job_queue SET lease_owner=$1,lease_until=now()+interval '90 seconds',attempts=attempts+1 WHERE id=(SELECT id FROM app_job_queue WHERE available_at<=now() AND (lease_until IS NULL OR lease_until<now()) AND ($2::uuid[] IS NULL OR organization_id=ANY($2::uuid[])) ORDER BY available_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING id,organization_id,attempts,capacity_deferrals`,
    [owner, organizations],
  );
