...
def build_findings(scorecard, manifest, complete):
    ...
    lines += ['', 'Every recorded attempt is retained; the final result is not a best-of selection. The worker retry policy from the frozen source for this run was used. Permanent-input handling may differ between versions; the observed attempt counts above remain authoritative.', '', '## Relevance classification', '', 'Confusion counts use the stored result’s relevance decision. Unavailable outputs are listed separately, not folded into false positives or false negatives; they still affect financial extraction coverage.', '']
    ...
    transport = scorecard.get('modelTransport') or {}
    transport_note = (f"Raw local model transport was recorded: {transport.get('chatRequestsAllAttempts')} chat requests, including {transport.get('chatRequestsWithImagesAllAttempts')} with images across all attempts. See the separate model-attempt integrity audit for exact byte reconstruction." if transport.get('recordingAvailable') else 'Raw local model transport was not recorded; engine pins, inventory, processor requests/results and trace counts were retained.')
    vision_note = f"Stored outputs report {transport.get('storedOutputVisionRequestTraces', 0)} image-input trace events. Model capability, actual image submissions, valid structured responses and grounded financial correctness are distinct measurements. Linux OCR requires separate validation."
    lines += ['', 'The Qwen alias in this experiment is configured for CPU execution; Gemma uses the existing deployment GPU configuration. Model size, quantization, device routing and warm-cache order differ. These sequential observations on shared local hardware are not a controlled CPU/GPU or model speed ranking. ' + transport_note, '', '## What these results do not measure', '', '- The corpus is fictional, authored with its gold labels, and not independently adjudicated. It does not establish production accuracy, multilingual coverage or general OCR robustness.', '- No extraction was automatically accepted or posted. Review decisions, ledger correctness, economic-event posting deduplication, consolidation accounting and stress-test accuracy are separate checks, not inferred from this benchmark.', '- Holdings look-through, manager relationships, knowledge-index proposal acceptance and total exposure are not scored by the six-field extraction comparison. Undisclosed weights must remain unknown.', '- The fake provider exercises production collection mechanics; it does not prove real mailbox connectivity, OAuth authorization, provider rate limits or delivery reliability.', '- ' + vision_note, '- No cloud model was called. This comparison does not establish cloud-versus-local accuracy or certify an air-gapped deployment.', '', f'Frozen corpus manifest SHA-256: {md(scorecard["manifestSha256"])}.', '']
    return '\n'.join(lines)
