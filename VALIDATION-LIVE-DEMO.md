...
In [run 34403006395](https://github.com/Blacklord100/aster-family-office/actions/runs/34403006395), both images built, their runtime probes passed, and all 589 processor tests passed inside the network-disabled image. The actual processor used authenticated upstream Tesseract 5.5.3 and libtiff 4.7.2; Pillow 12.3.0 was rebuilt against that same fixed TIFF library. Source signature/hash verification, installed native extension hashes, PNG/JPEG/compressed-TIFF round trips and real OCR passed. The complete web image passed its high/critical scan, and the repository secret scan passed independently.

The processor scan covered all 27 copied native package identities and all 32 locked Linux Python packages. It still reported three high-severity advisories with no fixed Debian version, despite the verified upstream replacements. The strict release gate remains blocked; no finding has been suppressed or relabeled. [The processor release-blocker record](operations/processor-release-blockers.md) identifies the exact packages, upstream fixes and outstanding scanner-coverage qualification. The [compact Linux qualification receipt](validation/linux-qualification-2026-09-09.json) records the tested commit, image identity, counts, findings and hashes of the retained full scanner/manifest artifacts. This Linux image qualification is separate from the native macOS Gemma demonstration.
